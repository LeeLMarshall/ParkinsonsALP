...
pC <-
  ...
  scale_y_reverse(breaks = 1:10, labels = pd$Gene) + 
...
pD <- 
  ...
  ggplot(., aes(AgingEffect, PDEffect)) + 
    ...
    stat_cor(method = "pearson", size = 2.2) +
    facet_grid(Type ~ .) +
    scale_x_continuous(
...
pd2 <- computeTrends(X, pfcfit,
  ...
  .[, Type := "PFC"] %>%
  .[, Group := factor(Group, levels = c("CTRL", "PD"), labels = c("Control", "PD/LBD"))]
...
pF <- predicted[, 
  ...
  ggplot(., aes(Group, `P-A`, color = Group)) + 
    ...
    stat_compare_means(method = "t.test", size = 2.2, 
      label.y = 50,
