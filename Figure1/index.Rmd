...
# rna <- fread(
#   "~/tmp/r1_Appendix_PDvsControls_RNAseq.csv")

enrichment <- readRDS("../Discover_GenomicEnrichment/enrichment_Appendix_PDvsControls_Padlock.RDS")
# enrichment <- readRDS("~/mnt/VAI/PD2018/Discover_GenomicEnrichment/enrichment_Appendix_PDvsControls_Padlock.RDS")
...
p_load("TxDb.Hsapiens.UCSC.hg19.knownGene")
p_load("org.Hs.eg.db")
genome <- "hg19"
...
ggdraw() +
  ...
  draw_plot(pD + guides(fill=FALSE), 
    x = 0.28, y = -0.02,  width = .3, height = .65) +
  draw_plot(legend, x = 0.20, y = 0.41, width = 0.2, height = 0.2) +
  draw_plot_label(
    label = c("A", "B", "C", "D", "E"), 
    size = 10,
...
**(A)** DNA modification manhattan plot. Highlighted in orange are the PD GWAS implicated genes targeted by significant DMCs (q < 0.05). Green hyper-methylation, blue hypo-methylation. **(B)** Distribution of hyper- and hypo-methylated significant DMCs (green and blue, resp.) versus the rest (grey). Fisher's exact test used to get the p value. **(C)** Enrichment of genomic elements with significant DMCs. Filled circles represent nominally significant enrichment (p < 0.05) obtained using Fisher's exact test. Errorbars indicate 95\% confidence intervals. **(D)** Enrichment of pathways with padlock target genes having DMC within 300kb. Dark bars are significant. **(E)** Number and fraction of up- and down- regulated genes (green and blue, resp.) belonging to a given pathway. **(G)** DAVID enrichment.